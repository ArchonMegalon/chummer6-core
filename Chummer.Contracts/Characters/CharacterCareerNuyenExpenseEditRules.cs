namespace Chummer.Contracts.Characters;

/// <summary>
/// Stable saved identity and exact editable fields behind CharacterCareer's Nuyen expense editor.
/// </summary>
public sealed record CharacterCareerNuyenExpenseEntry(
    Guid ExpenseId,
    DateTime ExpenseDateLocal,
    decimal Amount,
    string Reason,
    bool Refund,
    bool ForceCareerVisible,
    string NuyenUndoType,
    bool AmountEditable);

public sealed record CharacterCareerNuyenExpenseEditResult(
    CharacterCareerNuyenExpenseEntry Expense,
    decimal NuyenDelta);

public static class CharacterCareerNuyenExpenseEditRules
{
    public const decimal MaximumAmount = 9_999_999m;
    public const int MaximumReasonLength = 32_767;
    public static readonly DateTime MinimumDate = new(1753, 1, 1);
    public static readonly DateTime MaximumDate = new(9998, 12, 31, 23, 59, 59);

    public static bool IsAmountEditable(string? nuyenUndoType)
        => string.Equals(nuyenUndoType, "ManualAdd", StringComparison.Ordinal)
            || string.Equals(nuyenUndoType, "ManualSubtract", StringComparison.Ordinal);

    public static bool TryCreateEntry(
        Guid expenseId,
        DateTime expenseDateLocal,
        decimal amount,
        string? reason,
        bool refund,
        bool forceCareerVisible,
        string? nuyenUndoType,
        out CharacterCareerNuyenExpenseEntry? entry)
    {
        entry = null;
        string normalizedReason = reason ?? string.Empty;
        string normalizedUndoType = string.IsNullOrWhiteSpace(nuyenUndoType)
            ? "AddCyberware"
            : nuyenUndoType.Trim();
        if (expenseId == Guid.Empty
            || expenseDateLocal < MinimumDate
            || expenseDateLocal > MaximumDate
            || amount < -MaximumAmount
            || amount > MaximumAmount
            || normalizedReason.Length > MaximumReasonLength)
        {
            return false;
        }

        entry = new CharacterCareerNuyenExpenseEntry(
            expenseId,
            DateTime.SpecifyKind(expenseDateLocal, DateTimeKind.Unspecified),
            amount,
            normalizedReason,
            refund,
            forceCareerVisible,
            normalizedUndoType,
            IsAmountEditable(normalizedUndoType));
        return true;
    }

    public static bool TryEdit(
        CharacterCareerNuyenExpenseEntry current,
        decimal amount,
        string? reason,
        DateTime expenseDateLocal,
        out CharacterCareerNuyenExpenseEditResult? result)
    {
        ArgumentNullException.ThrowIfNull(current);
        result = null;
        string normalizedReason = reason ?? string.Empty;
        DateTime normalizedDate = DateTime.SpecifyKind(expenseDateLocal, DateTimeKind.Unspecified);
        decimal minimumAmount = current.Amount < 0m
            ? -MaximumAmount
            : current.Amount == 0m
                ? 0m
                : 1m;
        if (normalizedReason.Length > MaximumReasonLength
            || normalizedDate < MinimumDate
            || normalizedDate > MaximumDate
            || amount < minimumAmount
            || amount > MaximumAmount
            || (!current.AmountEditable && amount != current.Amount))
        {
            return false;
        }

        CharacterCareerNuyenExpenseEntry updated = current with
        {
            ExpenseDateLocal = normalizedDate,
            Amount = amount,
            Reason = normalizedReason
        };
        result = new CharacterCareerNuyenExpenseEditResult(
            updated,
            current.AmountEditable ? amount - current.Amount : 0m);
        return true;
    }
}
