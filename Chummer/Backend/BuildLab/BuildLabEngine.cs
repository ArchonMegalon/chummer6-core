...
namespace Chummer.Backend.BuildLab
{
    ...
    {
        private static readonly string[] KnownRoleTags = new[]
        {
            "matrix-specialist",
            "street-samurai",
            "infiltrator",
            ...
            return GenerateBuildVariants(characterId, roleTags)
                .Select(variant =>
                {
                    ...
                    BuildVariantScore? earlyConsistency = projection.Steps
                        .FirstOrDefault()?
                        .Scores
                        .FirstOrDefault(score => string.Equals(score.MetricId, "consistency", StringComparison.Ordinal));
                    BuildVariantScore? lateCeiling = projection.Steps
                        .LastOrDefault()?
                        .Scores
                        .FirstOrDefault(score => string.Equals(score.MetricId, "ceiling", StringComparison.Ordinal));
                    RulesetExplainParameter[] tradeoffSummaryParameters = new[]
                    {
        ...
        private static string ExtractTag(string variantId)
        {
            ...
            string stem = variantId.Substring(0, lastDash);
