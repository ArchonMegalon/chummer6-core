...
public sealed class DefaultBuildGhostRuleAuthorityResolver : IBuildGhostRuleAuthorityResolver
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            // IReadOnlyList can retain caller-owned mutable storage. Capture it
            // before any await; the normal exact primitive validation still applies.
            request = request! with { Arguments = Array.AsReadOnly(request!.Arguments.ToArray()) };
        }
        catch (Exception) when (!cancellationToken.IsCancellationRequested)
        {
            return Failure(BuildGhostRuleAuthorityStatuses.Unresolved, "typed-arguments-invalid");
        }

        BuildGhostActiveRuleAuthority? active;
        try
        {
            active = CaptureAuthority(await _subjectAuthority.ResolveAsync(
                    request!.OwnerId,
                    request.WorkspaceId,
                    request.SubjectId,
                    cancellationToken)
        ...
        try
        {
            IReadOnlyList<SourceAnchor>? returnedAnchors = await _sourceAnchors.ResolveAsync(active, intent, cancellationToken)
                .ConfigureAwait(false);
            // Validate and later return the same detached observation, not an
            // adapter's retained list that can change during subsequent awaits.
            sourceAnchors = returnedAnchors is null ? null : Array.AsReadOnly(returnedAnchors.ToArray());
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        {
            throw;
        ...
        // A context reference and a valid calculation trace bind the observation,
        // not the lifetime of the workspace or its authorization. Reacquire the
        // subject after both asynchronous dependencies before exposing any facts.
        // No owner/workspace lease may be retained across those awaits.
        BuildGhostActiveRuleAuthority? current;
        try
        {
            cancellationToken.ThrowIfCancellationRequested();
            current = CaptureAuthority(await _subjectAuthority.ResolveAsync(
                    request.OwnerId,
                    request.WorkspaceId,
                    request.SubjectId,
                    cancellationToken)
        ...
        if (current is null
            || !IsActiveAuthorityValid(current)
            || !BindingsEqual(active.Binding, current.Binding)
            || !active.ActiveSourcebookIds.ToHashSet(StringComparer.Ordinal)
                .SetEquals(current.ActiveSourcebookIds))
        {
            return Failure(
                BuildGhostRuleAuthorityStatuses.Stale,
    ...
    private static BuildGhostActiveRuleAuthority? CaptureAuthority(BuildGhostActiveRuleAuthority? authority)
        // IReadOnlyList does not guarantee immutable storage. Preserve the first
        // observed membership even if the provider reuses its list on the reread.
        => authority is { ActiveSourcebookIds: not null }
            ? authority with { ActiveSourcebookIds = Array.AsReadOnly(authority.ActiveSourcebookIds.ToArray()) }
            : authority;
