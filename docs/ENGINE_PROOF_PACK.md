...
- an `oracle_suite_summary` block that publishes the required suite count, passed suite count, covered rulesets, promoted-desktop release scope, and checked-in golden fixture totals for the golden oracle lane
- that `oracle_suite_summary.required_golden_fixture_count` stays pinned to the canonical release expectation instead of echoing the currently published suite rows, so a narrowed fixture set cannot redefine the required proof floor
- that `oracle_suite_summary` only reports `coverage_status=passed` when the published suite ids exactly match the required suite ids, so duplicate suite rows cannot silently replace a missing required lane
- that `oracle_suite_summary` also publishes missing, unexpected, and duplicate suite ids, so a failed golden-oracle release summary stays explainable without re-deriving the mismatch by hand
- per-suite oracle metadata including `coverage_focus`, `rulesets`, `release_scope`, `fixture_count`, explicit checked-in `golden_fixtures`, `golden_fixture_count`, and `total_fixture_count`, so release consumers can see which veteran-replacement lane each proof row is carrying and verify each suite stays bound to the promoted desktop release through governed contract fixtures instead of ambient repo state
...
- that `performance_budget_summary` only reports `coverage_status=passed` when the published budget ids exactly match the required budget ids, so duplicate budget rows cannot silently replace a missing release gate
- that `performance_budget_summary` also publishes missing, unexpected, and duplicate budget ids, so a failed release-budget summary remains explainable at the pack top level
- per-budget release metadata including `release_gate`, `scenario`, and `release_scope`, so explain and performance budgets read as release-bound workloads instead of anonymous thresholds
...
- top-level queue closure instruction mirrors for `completion_action`, `do_not_reopen_reason`, and `queue_closure_field_drift`, so closeout consumers can read the completed-package posture without depending on nested `successor_wave_authority` paths
- queue closeout proof that the successor queue row is marked `status: complete`, pins frontier `3227666051`, cites landed commit `00800059`, requires `completion_action: verify_closed_package_only`, carries an explicit package-specific `do_not_reopen_reason`, and lists the proof anchors for this generated pack, generator, tests, and documentation
- queue-mirror closeout parity for `completion_action` and the full `do_not_reopen_reason` text, so Fleet and design queue rows cannot quietly diverge on the closure instruction while still satisfying token-presence checks
...
- local proof for commit `8f4702a5`, which tightens the current release-bound proof discipline guard for executable budget workloads, exact import-oracle coverage, malformed release-channel rows, and copied supervisor-helper instructions
- local proof for commit `aeeeaf6e`, which tightens package-commit citation proof so registry, Fleet queue, and design queue package-local commit evidence must resolve locally
- queue and registry closeout proof for commit `c84b251f`, which pins the package-commit citation guard as an explicit local proof floor
- local proof for commit `29b17c68`, which keeps oracle-suite and performance-budget summaries release-bound and explainable by suite, gate, and workload
- local proof for commit `262030df`, which fail-closes queue closure-instruction drift and malformed aggregate import-oracle coverage
- import-oracle discipline aggregate coverage copied from `IMPORT_PARITY_CERTIFICATION.generated.json`, so release-bound consumers can read the exact covered-source counts and `coverage_percent` that already gate pass/fail
- local git commit proof that the cited M104 guard commits, including guards `56048971`, `769e7259`, `d4b3b0ba`, `a2173476`, `dafc1205`, `65df3894`, `4a56911d`, `4b124997`, `2187db33`, `b488d109`, `b6fddf74`, `3b9a29c2`, `f6608678`, `a3cbb548`, `df0527b2`, `8574f63f`, `6b3a662c`, `3b63478f`, `31c75c02`, `ef46554c`, `0771b7ea`, `fdb6a273`, `d2ee91a9`, `cd30503f`, `e10f2739`, `e7d4270e`, `bbc877d7`, `56ff7283`, `7ae79416`, `a613bdb2`, `353921e7`, `9de2455b`, `d8e826a3`, `7a1f0e7c`, `d464cfab`, `a1a2d956`, `abf63719`, `bbc7fba8`, `a1a1d505`, `18d03556`, `77cb53cf`, `f914ce6a`, `3c242c2f`, `c2872b40`, `ea449f7b`, `18365058`, `5031ee41`, `cbce6a19`, `71441924`, `df1330b4`, `6610ff2e`, `2c8742ad`, `5baebb73`, `40babebd`, `22171b35`, `c6fbd75f`, `96eca660`, `05e47cff`, `93d06011`, `31aec38a`, `ceccc309`, `5dff1a2e`, `2301a043`, `5c75316f`, `28be988f`, `c6a2ee8e`, `6684fc89`, `ccbfc6b2`, `2a3ebcb9`, `7501f49a`, `ac961fe1`, `36311e16`, `db3cc033`, `be5755a6`, `8ffec2b1`, `ee9d88b1`, `eacefaf2`, `e4e502a1`, `1f2c5724`, `1bcb9b7e`, `e04d7b88`, `58656418`, `73638668`, `a404b474`, `51bb2d8f`, `507f1f6b`, `43638c3e`, `b0776012`, `5f50cb7b`, `c58d18e1`, `67e0f654`, `d584120b`, `39c875fd`, `f1b6c5ca`, `faf14925`, `64b8f873`, `06a2e06a`, `6d25fb18`, `cc6cf25b`, `bb9af238`, `44512fcf`, `4db6d429`, `adc72a7e`, `5e808a1b`, `c323b4ad`, `7a432bc3`, `c124e4af`, `5a649e57`, `af67ecfd`, `870be707`, `498dff3d`, `b8000b80`, `ecbb466c`, `a2c8ad9f`, `2c98f61c`, `2e4e8e81`, `b5d46938`, `c1300863`, `8f4702a5`, `aeeeaf6e`, `c84b251f`, `29b17c68`, and `262030df`, resolve in this repository before the release-bound proof pack can pass
- registry and queue package-commit citation proof, so any `/docker/chummercomplete/chummer-core-engine commit ...` evidence added to the M104 registry row, Fleet queue row, or design-owned queue row must resolve locally even when it has not been manually added to the fixed guard list yet
...
- supervisor retry-instruction hygiene, so copied instructions such as "do not run supervisor status or eta helpers" cannot satisfy registry, queue, closeout, or import-certification proof
...
Executable workload evidence means the required workload name is present both on a BenchmarkDotNet `[Benchmark(Description = "...")]` method and in the `CreateBudgetWorkloads()` factory row that the release budget runner executes.
The proof generator fails closed when a required budget lane is missing from either the budget file or either executable benchmark workload source anchor, or when the budget file duplicates a required workload name that could mask stale or conflicting release-budget thresholds.
...
Those command and evidence rows must be unique and must not include extra non-release-bound rows or cite active-run, task-local, supervisor, supervisor-status-or-ETA-helper, OODA, handoff telemetry, previous-attempt or previous attempt prompt text, or implementation-only retry prompt text as proof, including percent-encoded, URL form-encoded, HTML-escaped, or separator-obfuscated copies of those tokens.
Their coverage counts and percent must be complete JSON integer coverage rather than string-encoded numbers, fractional values, integer-valued floats, booleans, under-100-percent or over-100-percent summaries, adjacent-oracle-omitting totals, or a later duplicate row masking an earlier incomplete receipt.
...
Adjacent oracle rows such as Genesis or CommLink6 must include explicit `sources_covered` and `sources_expected` counts, and the proof pack fails closed unless those counts show exact complete coverage.
...
Import and adjacent oracle rows must report `sources_covered == sources_expected`; overcounted rows fail closed instead of letting inflated source counts mask a narrowed oracle receipt.
