...
- `.codex-studio/published/ENGINE_PROOF_PACK.generated.json` mirrors `queue_completion_action`, `design_queue_completion_action`, both package-specific `do_not_reopen_reason` values, and `queue_closure_field_drift=[]` at the top level, so closeout consumers can verify the completed-package posture without traversing nested authority fields.
- `.codex-studio/published/ENGINE_PROOF_PACK.generated.json` publishes `oracle_suite_summary.coverage_status=passed`, `oracle_suite_summary.covered_rulesets=[sr4, sr5, sr6]`, `oracle_suite_summary.release_scope=promoted_desktop_release`, and `published_golden_fixture_count=10`.
- Every required oracle suite row keeps release-bound `coverage_focus`, `rulesets`, `release_scope=promoted_desktop_release`, `fixture_count`, explicit checked-in `golden_fixtures`, and `golden_fixture_count` metadata so the golden oracle lane stays explainable by suite.
...
- Every required performance budget row keeps release-bound `release_gate`, `scenario`, and `release_scope=promoted_desktop_release` metadata so the benchmark lane stays bound to promoted desktop workloads instead of anonymous thresholds.
...
- Local commit proof includes `5a649e57`, `c01dfa10`, `1a98d904`, `af67ecfd`, `870be707`, `498dff3d`, `ecbb466c`, `a2c8ad9f`, `2c98f61c`, `2e4e8e81`, `b5d46938`, `c1300863`, `8f4702a5`, `aeeeaf6e`, `c84b251f`, `29b17c68`, and `262030df`, the M104 proof pack handoff, queue-mirror parity guard, package-commit citation guard, package-commit citation floor, release-summary guard, closure-instruction guard, and current proof-guard anchors.
...
- Registry and queue evidence do not cite worker-run scaffolding such as execution discipline, required order, first action rule, assigned successor queue package, assigned slice authority, successor frontier detail, or execution rules inside this run as release proof.
...
- `.codex-studio/published/ENGINE_PROOF_PACK.generated.json` surfaces that import-certification aggregate coverage object unchanged, so release verification can read `sources_covered`, `sources_expected`, and `coverage_percent` directly from the engine proof pack.
- `.codex-studio/published/ENGINE_PROOF_PACK.generated.json` keeps `published_import_oracle_names` and `published_adjacent_oracle_names` bound to the assigned release oracle set, and both unexpected-name lists remain empty.
- `.codex-studio/published/ENGINE_PROOF_PACK.generated.json` keeps `malformed_import_oracle_rows=[]` and `malformed_adjacent_oracle_rows=[]`, so anonymous or non-object oracle rows cannot hide beside the required release-bound oracle set.
