...
public sealed partial class Sr6CreationFoundationTests
{
    ...
    public void Karma_specialty_on_existing_mystic_adept_preserves_all_domain_anchors(bool withKnowledge, bool withQualities, bool withMetatypeUpgrade, bool withContacts, bool withGear, bool withLifestyle)
    {
        ...
        if (withLifestyle) seed = seed with { Lifestyle = new("low", 1) };
        var request = fixture.Request(seed);
        var quote = fixture.Preview(seed);
        Assert.HasCount((withKnowledge ? 10 : 9) + (withQualities ? 1 : 0) + (withMetatypeUpgrade ? 1 : 0) + (withContacts ? 1 : 0) + (withGear ? 1 : 0) + (withLifestyle ? 1 : 0), quote.SourceAnchorIds);
